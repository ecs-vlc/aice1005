...
\begin{PauseHighLight}
  ...
  \item Eventually, we will reach $m$ constraints which defines a vertex
...
\begin{PauseHighLight}
  ...
  \item $m$ constraints must be satisfied to be at a vertex of feasible
    region\pause
...
\begin{PauseHighLight}
  ...
  \item To do this for each additional constraint we add a new
    ...
    \begin{align*}
     \forall\, k\in\{1,2,\ldots, m\} \quad\quad\pauselevel{=3 :4}\pause
     \xi_k + \pauselevel{build =4}\pause\sum_i A_{ki} x_i &= b_k  \geq 0 \pauselevel{=3 :4}\pause
    \end{align*}
  \item We then can find a basic feasible solution by setting $x_i=0$ so
    \begin{align*}
      \xi_k &= b_k  \quad \forall\, k\in\{1,2,\ldots, m\}\pauselevel{=5}\pause
    \end{align*}
  \end{itemize}
\end{PauseHighLight}
