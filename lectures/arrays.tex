...
\item We can add elements\pause{}\pauselevel{=7}
...
\item The number of elements copied is
  \begin{align*}
    ...
    \pause = n \, (2^{m}-1)\pause
  \end{align*}
\item Total number of operations is\pause\pauselevel{=9} (using
  $\lceil \log(a) \rceil < \log(a) +1$)\pause\pauselevel{=7}
  \begin{align*}
...
\begin{slide}
\section[-1]{Insertion and Deletion}

\begin{PauseHighLight}
  \begin{itemize}
  \item In C++ \jl$vector<T>$ is a resizable array and used a lot\pause
...
\begin{PauseHighLight}
  ...
  \item Amongst a number of issues that make C++ dangerous are
    ...
    \item Writing to parts of memory that you should not\pause
  ...
  \item However, by using existing data structures (STL) and following
    established programming patterns these don't have to be an
    issue\pause
    \begin{itemize}
    \item Use \jl$vector<T>$, \jl+set<T>+, etc.  not raw arrays\pauseb
    \item Use standard for loops (iterators)\pauseb
    \end{itemize}
  \end{itemize}
\end{PauseHighLight}
...
\begin{cpp}
  ...
  class container {
    ...
    public:
      ...
      $\(\sim\)$container(unsigned n) {delete[] data;} $\pauselevel{=3}\pauseb$
  };$\pauselevel{=1}\pause$
...
\begin{PauseHighLight}
  \begin{itemize}
  \item In C and C++ the following will compile and run
    \begin{cpp}
      int *array = new int[4];
      int *a = new int[2];
...
\begin{PauseHighLight}
  ...
  \item insertion and deletion from the middle of an array are expensive, $O(n)$\pause
