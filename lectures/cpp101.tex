...
\begin{PauseHighLight}
  ...
  \item C facilitated this through C structures \texttt{struct}
    \begin{cpp}
      ...
      }; // $End the structure with a semicolon\pause$
...
\begin{PauseHighLight}
  ...
  \item We can do this on the fly by storing
    ...
    \end{align*}
  \item Given $X_{n+1}$ we can update our data using: $\Delta =
    \frac{X_{n+1} - \hat{\mu}_n}{n+1}$
    \begin{align*}
      \hat{\mu}_{n+1} &= \hat{\mu}_n + \Delta
      &
...
\begin{PauseHighLight}
  ...
  \item In C we can use a \texttt{struct} to keep this data together
    ...
    \end{cpp}
  \item We can write functions that update the Sos structure
    \begin{cpp}
    ...
    \end{cpp}
  \item \jl$struct$s help keep information together, but can we do better?\pause
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item Classes are richer user defined datatype than structs\pause
  ...
  \item You are familiar with classes from python and they are very
    ...
    python\pauseb
  \item C++ has grown since 1985, adding templates and a lot of nice
...
\begin{PauseHighLight}
  \begin{itemize}
  \item Define interface in header file \texttt{sos.h}\pause
  \end{itemize}
\end{PauseHighLight}

\begin{cpp}
  #ifndef __SOS_H__
  #define __SOS_H__$\pause$
...
\begin{PauseHighLight}
  ...
  \item You include libraries using include statements
    \begin{cpp}
      #include <iostream>
      #include <vector>$\pause$
    \end{cpp}
    \vspace*{-1cm}
  \item Same as C, but the C++ built in libraries don't have
    ``.h''\pause
  \item These are known as the standard library or the standard
    template library (STL)\pause
  \item There is a naming convention, that libraries built into the
    system are called \jl$<library>$ while libraries you write are
    called \jl$"library.h"$\pause
  \end{itemize}
\end{PauseHighLight}
...
\end{cpp}
\item To call a member function, \jl+func()+, of class \jl$w$ you use
  \begin{cpp}
    w.func()$\pause$
  \end{cpp}
  \item To call a member function of \jl+wpt+ use either
    \begin{cpp}
(*wpt).func();  // $dereference object and call member function\pause$
wpt->func();    // $easy to type\pauseb$
    \end{cpp}
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  ...
  \item There are many uses of references, one is so we can make
    functions change the value of their arguments
    \begin{cpp}
