...
\begin{PauseHighLight}
  ...
  \item In C we would typically use a for-loop to iterate over an
    array
    \begin{cpp}
int n = 10;                        // $\small \textit{size of array}$
int* begin = malloc(n*sizeof(10)); // $\small \textit{malloc returns beginning of array} \pause$
int* end = begin + n;              // $\small \textit{address past end of array}\pause$

int sum = 0;
for(int* pt = begin; pt != end; pt++) {
  sum += *pt;                     // $\small \textit{need to dereference pointer}\pause$
}
    \end{cpp}
  \item Ugly\pauseb, but efficient\pauseb
  \item Acts as a prototype for C++ iterators\pauseb
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item In addition the iterator class we should have two methods
    \begin{itemize}
    \item \jl+begin()+
    \item \jl+end()+
    \end{itemize}
...
private:
  ...
  struct iterator {          // An iterator class $\pause$
    ...
    iterator& operator++() {  // next entry ++it
      entry = entry->next;
...
\begin{PauseHighLight}
  ...
  \item C++ has a pre-increment opeator \jl$++a$ and a post-increment
    operator \jl$a++$\pause
  \item The pre-increment operator increments \jl$it$ and returns the
    incremented version, e.g.
    \begin{cpp}
iterator& operator++() {++it; return it;}  // defines ++it$\pause$
    \end{cpp}
  \item The post-increment operator copies \jl$it$ increments it and
    returns the copy, e.g.
    \begin{cpp}
T& operator++(int) {T v = it->value; ++it; return v;} // defines it++$\pause$
    \end{cpp}
...
\begin{PauseHighLight}
  ...
  \item This requires us to implement the decrement operators
    \begin{cpp}
    iterator& operator--();     // implements --obj
