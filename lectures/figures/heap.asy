...
struct Heap {
  ...
  path edge(int node1, int node2) {
    real u = 0.2;
    real v = 1.0 - u;
    return v*pos(node1)+u*pos(node2)--u*pos(node1)+v*pos(node2);
  }
  
  void highlight_edge(int node1, int node2, arrowbar arrow=None) {
    draw(edge(node1, node2), red, arrow);
  }
  ...
  int pop(bool show = false) {
    ...
    if (show) {
      ...
      graph();
      ship();
      pair p1 = pos(0)+0.8*(18,10);
      pair p2 = 0.5*(pos(0)+p1) + (0.2,0);
      draw(pos(0)..p2..p1, gray, Arrow);
      draw(pos(count-1)+(0.05,0.2)..0.5(pos(0)+pos(count-1))+(3.2,0)..pos(0)+(0.5,-2.5), red, Arrow);
	...
	highlight(child);
	highlight_edge(child, parent);
	list_edge(parent, child, Arrow);
	if (child+1<count) {
	  highlight_edge(child+1, parent);
	  highlight(child+1);
	  list_edge(parent, child+1, Arrow);
	}
