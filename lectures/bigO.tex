...
\item If we don't know then we can at least say that the run time is in
  $O(n^2)$\pause---we assume the worst\pauseb, but the worst may never happen\pauseb
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  \begin{itemize}
  \item It is often easy to obtain a lower bound on a particular
    algorithm\pause, although getting a tight general lower bound is often
    very difficult\pauseb
...
\item Thus we know this algorithm is in $\Omega(n)$\pause---we assume
  the best\pauseb, but the best may never happen\pauseb
  \end{itemize}
\end{PauseHighLight}
...
\begin{PauseHighLight}
  ...
  \item This is true if
    \begin{itemize}
    \item $T(n) = 2^n$ \pause\pauselevel{=3} \hspace{3.5cm} $\log(T(n)) =
      n\,\log(2)$ \pauseb
    \item \pauselevel{=2}$T(n) = 6.1\,\e{0.003n}$\pause\pauselevel{=3} \hspace{1cm} $\log(T(n)) =
      0.003\,n + \log(6.1)$ \pauseb
    \item \pauselevel{=2}$T(n) = n \, 10^n$\pause\pause\pauselevel{=3} \hspace{2.7cm} $\log(T(n)) =
...
\begin{PauseHighLight}
  ...
  \item To prove
    ...
    \begin{align*}
     \hspace{-2em} \gcd(m,n) &= \gcd(n,\rem(m,n)) = \gcd(\rem(m,n),\rem(n,\rem(m,n)))\hspace{1em}\pause
    \end{align*}
    \item The proof follows by showing that for all $k<n$ we have
      $\rem(n,k)<n/2\leq k$ for $k>n$ then $\rem(n,k)=n$\pause
    \end{itemize}
  \item Thus $T(n)<T(\lfloor n/2 \rfloor)+2$\pause
  \end{itemize}
\end{PauseHighLight}
